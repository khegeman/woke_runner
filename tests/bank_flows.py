...
class BankTest(FuzzTest):
    ...
    def deposit(self, random_input: BankInput) -> None:
        ...
        with may_revert() as e:
            ...
            assert balance + random_input.amount == self._bank.accounts(
                random_input.user
            )
            assert tx.events[0] == Bank.Deposit(
                get_address(random_input.user), random_input.amount
            )
            assert not overflow
    ...
    def withdraw(self, random_input: BankInput) -> None:
        ...
        with may_revert() as e:
            ...
            assert tx.events[0] == Bank.Withdraw(
                get_address(random_input.user), random_input.amount
            )
            self.bankMirror[random_input.user] -= random_input.amount
        if e.value is not None:
