...
from typing import Callable, DefaultDict, List, Optional, Type,Dict,Iterable,get_type_hints,Union
...
def stateful_generator(cls: type, flow_count : int):
   ...
   flows = [Flow(name = random.choice(flow_names), properties=[]) for i in range(0,flow_count)]

   return fuzz_generator(cls, flows)


def fuzz_test(test : FuzzTest, sequences_count : int = 1,flow_name : Union[str,Iterable[str]] ="", flows : Iterable[Flow] = None):
    if flows is None:        
        flows = [Flow(name=flow_name,properties=[])] if type(flow_name) is str else [Flow(name=name,properties=[]) for name in flow_name]
...
def stateful_test(test : FuzzTest, sequences_count : int, flow_count : int):
    for i in range(0,sequences_count):
        test._sequence_num = i
        run(test, bound_flows=stateful_generator(test,flow_count=flow_count), properties=[])
